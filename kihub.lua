-- [[ KIIHUB- INDEX LOADER V.0.60 (INJECT TO UI) ]] --
...
pcall(function() if getgenv().KIIIndexUI then getgenv().zonIndexUI:Destroy() end end)

-- [[ SETUP UI UTAMA ]] --
local ScreenGui = Instance.new("ScreenGui"); ScreenGui.Name = "KIIHUBIndex"; pcall(function() ScreenGui.Parent = CoreGui end); if not ScreenGui.Parent then ScreenGui.Parent = LP.PlayerGui end; ScreenGui.ResetOnSpawn = false; getgenv().ZonIndexUI = ScreenGui 
...
local KIIBtn = Instance.new("TextButton", ScreenGui); KIIBtn.BackgroundColor3 = Theme.Purple; KIIBtn.Position = UDim2.new(0.1, 0, 0.1, 0); KIIBtn.Size = UDim2.new(0, 50, 0, 50); KIIBtn.Text = "KII"; KIIBtn.TextColor3 = Color3.new(1,1,1); KIIBtn.Font = Enum.Font.GothamBlack; KIIBtn.TextSize = 22; KIIBtn.Visible = false; Instance.new("UICorner", KIIBtn).CornerRadius = UDim.new(1, 0)
...
local Title = Instance.new("TextLabel", Header); Title.Text = " KIIHUB " .. getgenv().HubVersion; Title.TextColor3 = Theme.Purple; Title.Font = Enum.Font.GothamBlack; Title.TextSize = 16; Title.Size = UDim2.new(0.5, 0, 1, 0); Title.BackgroundTransparency = 1; Title.TextXAlignment = Enum.TextXAlignment.Left
...
local function ToggleUI() Main.Visible = not Main.Visible; KIIBtn.Visible = not Main.Visible; if KIIBtn.Visible then KIIBtn.Position = Main.Position else Main.Position = KIIBtn.Position end end; MinBtn.MouseButton1Click:Connect(ToggleUI); KIIBtn.MouseButton1Click:Connect(ToggleUI)
